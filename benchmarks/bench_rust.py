"""Benchmark script for Rust-accelerated implementation.

Uses the same benchmarks as bench_python.py but imports through
the project's modules (which route through the Rust backend when available).
Loads the Python baseline from baseline_python.json and prints a comparison.
"""
...
# Detect backend
try:
    from schedgym._schedgym_rs import Heap as _RustHeap  # noqa: F401
...
def load_baseline() -> dict[str, dict]:
    """Load baseline_python.json and index by label."""
    baseline_path = Path(__file__).resolve().parent / "baseline_python.json"
...
def print_comparison(results: list[dict], baseline: dict[str, dict]):
    """Print a side-by-side comparison table."""
    print()
    print("=" * 90)
    print(f"  {'Benchmark':<48} {'Python':>10} {'Rust':>10} {'Speedup':>10}")
    print("=" * 90)

    for r in results:
        label = r["label"]
        rust_ms = r["mean_s"] * 1000
        base = baseline.get(label)
        if base:
            py_ms = base["mean_s"] * 1000
            speedup = py_ms / rust_ms if rust_ms > 0 else float("inf")
            arrow = "🟢" if speedup > 1.1 else ("🔴" if speedup < 0.9 else "⚪")
            print(
                f"  {label:<48} {py_ms:>8.2f}ms {rust_ms:>8.2f}ms  {arrow} {speedup:>5.1f}x"
            )
        else:
            print(f"  {label:<48} {'N/A':>10} {rust_ms:>8.2f}ms  {'—':>10}")

    print("=" * 90)

    # Summary
    total_py = sum(
        baseline[r["label"]]["mean_s"] for r in results if r["label"] in baseline
    )
    total_rust = sum(r["mean_s"] for r in results if r["label"] in baseline)
    if total_rust > 0:
        overall = total_py / total_rust
        print(
            f"\n  Overall: Python {total_py * 1000:.1f}ms → Rust {total_rust * 1000:.1f}ms"
            f"  ({overall:.1f}x aggregate speedup)"
        )
    print()
...
def main():
    print("=" * 90)
    print(f"  sched-rl-gym  Performance Benchmark  —  Backend: {BACKEND}")
    print("=" * 90)
    print()
    ...
    if baseline:
        ...
        print("\n  ⚠ No baseline_python.json found — run bench_python.py first.")

    # Save Rust results
    out_path = Path(__file__).resolve().parent / "results_rust.json"
