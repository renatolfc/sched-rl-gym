...
class CompactRmEnv(BaseRmEnv):
    ...
    def really_done(self) -> bool:
        return (
            len(self.scheduler.queue_admission) == 0
            and len(self.scheduler.queue_waiting) == 0
        )
    ...
    def state(self):
        ...
        ptr = 0
        if self.smdp:
            self._state_buffer[ptr : ptr + snapshots] = np.log(
                np.array(state[0]) + 1.0
            ) / np.log(self.time_limit)
            ptr += snapshots

        self._state_buffer[ptr : ptr + snapshots * 2] = (
            np.array([(e[0], e[1]) for e in state[proc_idx]], dtype=np.float32).reshape(
                (-1,)
            )
            / self.processors
        )
        ...
        if not self.ignore_memory:
            self._state_buffer[ptr : ptr + snapshots * 2] = (
                np.array(
                    [(e[0], e[1]) for e in state[mem_idx]], dtype=np.float32
                ).reshape((-1,))
                / self.memory
            )
            ptr += snapshots * 2
        ...
        remaining_work = (
            ...
            if running and self.maximum_work
            else 0.0
        )
        ...
        remaining_work_mem = (
            ...
            if running and self.maximum_work_mem
            else 0.0
        )

        queue_size = min(len(self.scheduler.queue_admission) / 1000.0, 1.0)
        time_left = 1 - np.log(self.scheduler.current_time + 1) / np.log(
            self.time_limit
        )
        ...
        try:
            ...
            next_free_arr = np.array(
                (
                    ...
                    (state[proc_idx][0][0] + next_free.requested_processors)
                    / self.processors,
                )
            )
        except ValueError:
    ...
    def _normalize_jobs(self, jobs):
        ...
        for i, job in enumerate(jobs):
            _sumdiv(
                ...
                * np.log(np.abs(job.submission_time) + np.e)
                - 1,
                np.log(self.time_limit),
            )
            _sumdiv(
                ret[i],
                1,
                np.sign(job.requested_time) * np.log(np.abs(job.requested_time) + np.e)
                - 1,
                np.log(self.time_limit),
            )
